...
class ExportService:
    ...
    def _find_reusable_export(self, request_hash: str, request_id: int) -> dict[str, Any] | None:
        with self.database.connect() as conn:
            return self.exports_repository.find_reusable_export(conn, request_hash, request_id)

    def _export_items(self, export_id: int) -> list[dict[str, Any]]:
        with self.database.connect() as conn:
            return self.exports_repository.list_export_items(conn, export_id)

    def _replace_export_items(self, export_id: int, exported_instances: list[dict[str, Any]]) -> None:
        with self.database.connect() as conn:
            self.exports_repository.replace_export_items(conn, export_id, exported_instances)
            conn.commit()
    ...
    def _upsert_export(
        ...
        with self.database.connect() as conn:
            export = self.exports_repository.upsert_export(
                conn,
                request_id,
                status,
                export_path,
